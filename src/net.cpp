...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "sCoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{ 0x4e84e217, 0x4b98fb36, 0xb47dc848
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. sCoin is probably already running."), addrBind.ToString().c_str());
        else
