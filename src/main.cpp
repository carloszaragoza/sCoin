...
uint256 hashGenesisBlock("0x8ebf472cafeb296bf6298627bb4abe0334ab3c69a626cbe3a979ecafdbfce33a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Scoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Scoin Signed Message:\n";
...
{
    // Scoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Scoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60 * 60; //restoring to 5 hours
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Scoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Scoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Scoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ebf472cafeb296bf6298627bb4abe0334ab3c69a626cbe3a979ecafdbfce33a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "inflationary-samtoshi nakamoto";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388674193;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385597064;
        ...
        assert(block.hashMerkleRoot == uint256("0x76b80857f0250a9df2630639f154ae7c969630e0e1a20051e2f7f6196390985f"));
...
unsigned char pchMessageStart[4] = { 0xf6, 0xbe, 0xb4, 0xd9 }; // Scoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ScoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ScoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ScoinMiner(CWallet *pwallet)
{
    printf("ScoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Scoin-miner");
    ...
    try { loop {
        ...
        printf("Running ScoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ScoinMiner, pwallet));
}
